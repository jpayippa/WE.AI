...
import { FaArrowUp, FaEdit, FaSave, FaTimes, FaSync, FaVolumeUp, FaHistory, FaTrash } from "react-icons/fa";
  ...
  const handleEditChatTitle = async (chatId, currentTitle) => {
    if (editingChatId === chatId) {
        ...
        if (trimmedTitle && trimmedTitle !== currentTitle) {
            const success = await updateChatTitle(session.user.email, chatId, trimmedTitle);
            if (success) {
